	...
	<table>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="file"><?php echo ($vol["filename"]); ?>
                    <?php if($vol["file"] !=''): ?><a href="javascript:;" class="download" data="<?php echo ($vol["id"]); ?>">【下载】</a><?php endif; ?>
                </td>
                <td class="content"><?php echo (msubstr($vol["content"],0,20)); ?></td>
                <td class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?></td>
                <<td class="status"><?php if($vol["isread"] == 0): ?><span style="color: violet">未读</span><?php else: ?><span style="color:red">已读</span><?php endif; ?></td>
                <td class="operate">
...
$(function(){
    ...
    $('.show').on('click',function(){
        ...
        var emailIsread = $(this).attr('data-isread');
        //layer代码
        layer.open({
            type: 2,
            title: emailTitle,
            ...
            shade: 0,
            area: ['550px', '90%'],
            content: '/index.php/Admin/Email/getContent/isrec/1/id/'+id, //iframe的url
            //表示当层或者窗口销毁的时候触发的回调函数
            end:function(){
                //刷新页面
                if(emailIsread == 0){
